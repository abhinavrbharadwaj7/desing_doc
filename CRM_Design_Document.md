...
|----------|------|------------|---------|
| R1 | 17/09/2025 | Abhinav R Bharadwaj | Updated Planning strategy for Customer Request Manager (CRM) |
| R2 | 12/11/2025 | Copilot Agent | Created comprehensive design document from R1 planning artifacts |

---

## 1. Summary

The Customer Request Manager (CRM) is a centralized platform for tracking and managing customer compliance and sustainability requests within the Acquis Compliance Platform. It replaces manual tracking methods with a unified tenant portal where all users can create, assign, monitor, and collaborate on customer requests. The system provides real-time visibility, transparent workflows, and comprehensive audit trails while maintaining strict tenant-level data isolation.

**Key Outcomes:**
- Centralized request tracking with unified tenant portal
- Role-agnostic access - all tenant users can manage requests
- Real-time dashboards with KPIs and visual analytics
...
3. Plan R2 enhancements: advanced workflows, notifications, integrations
4. Establish monitoring and SLA tracking
...
The CRM tool addresses the critical need for organized tracking of customer compliance and sustainability requests. Currently, many organizations rely on manual methods (spreadsheets, email threads) which lack centralization, accountability, and real-time visibility. The CRM provides a structured system where teams can efficiently intake, assign, track, and resolve customer requests while maintaining complete audit trails.

### 2.2 Scope

**In Scope:**
- Request creation with duplicate validation
- Assignment within tenant (no role restrictions)
...
- All Requests view with filtering and search
- My Tasks view for assigned requests
...
| **Request Type** | Category of request (e.g., Compliance Data, Sustainability Report, Audit) |
| **Regulation** | Specific compliance regulation referenced in request (REACH, RoHS, CMRT, etc.) |
...
### 4.1 User Flow: Creating and Resolving a Request
...
Navigate to Summary Dashboard (landing page)
    ↓
View KPIs and charts for tenant requests
    ↓
Click "All Requests" to see full list
    ↓
...
Request appears in "All Requests" and assignee's "My Tasks"
    ↓
Assignee updates status, adds comments, uploads files
    ↓
...
### 4.2 User Flow: Cancelling a Request

```
User navigates to "All Requests"
    ↓
...
|-----------|---------------|
| **Summary Dashboard** | Display KPIs, charts, and personal task summaries for quick insights |
| **All Requests View** | Comprehensive table of all tenant requests with filters, search, and actions |
| **Request Form** | Intake new requests with validation, duplicate detection, and file uploads |
| **My Tasks View** | Personalized view of requests assigned to logged-in user |
| **Cancel Request Popup** | Handle soft deletion with mandatory reason capture |
| **Export Service** | Generate CSV/Excel exports of request data |
| **Audit Logger** | Record all user actions with timestamp, user, and details |
| **File Storage Service** | Manage file uploads to Azure Blob Storage |
...
**Request Service:**
- Request lifecycle management (create, update, cancel)
- Duplicate validation based on customer + type + regulation
...
**React Components:**
- **Summary Dashboard:** KPI cards, donut charts, line charts, bar charts, heat maps
- **All Requests Table:** Sortable, filterable, searchable data grid with actions
- **Request Form:** Multi-field form with validation and file upload
- **My Tasks:** Filtered view of assigned requests
- **Cancel Popup:** Modal dialog with reason input

**State Management:**
- React Context API for global state (user session, tenant info)
...
- Bar chart: Request type breakdown
- Heat map: Regulation distribution
...
```javascript
{
  ...
  requestType: String,              // Type of request (e.g., "Compliance Data")
...
```javascript
{
  ...
  action: String,                   // "CREATE" | "UPDATE" | "CANCEL" | "EXPORT" | "UPLOAD"
